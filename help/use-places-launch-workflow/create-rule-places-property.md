...
* **退出POI**，此動作會在客戶離開您設定的POI時由Places SDK觸發。
...
>[!CAUTION]
>
>此範例假設您已建立美國所有咖啡店的 POI 資料庫。如需建立POI和資料庫的詳細資訊，請參閱 [建立POI](/help/poi-mgmt-ui/create-a-poi-ui.md) 和 *建立程式庫* 在 [管理多個程式庫](https://experienceleague.adobe.com/docs/places/using/poi-mgmt-ui/manage-libraries-in-the-places-ui.html).
...
1. 在事件區段中，按一下 **[!UICONTROL +新增]**. 事件會決定您何時要引發規則。
1. 在 **[!UICONTROL 副檔名]** 下拉式清單，選取 **[!UICONTROL 地點 — Beta版]**.
...
1. 在「條件」段落中，按一下 **[!UICONTROL +新增]**. 條件會決定要採取的動作必須符合哪些條件。
...
1. 選取動作型別，例如 **[!UICONTROL 傳送回傳]**.
   ...
   a.在 **[!UICONTROL URL]**，輸入回傳URL以進行Slack，例如， `https://hooks.slack.com/services/`.
...
使用Places Service地理圍欄專案與退出點來觸發Experience Platform Launch中的規則非常強大，但您也可以使用位置資料作為其他事件引發的條件。 例如，您可以讓行動核心追蹤動作事件觸發器準備就緒，可根據應用程式內的特定trackAction呼叫事件觸發。 根據此事件，您可在執行動作之前，為事件放置其他位置條件。 例如，購買時開啟應用程式內調查 `trackAction` 事件發生，但 **僅限** 如果使用者的目前位置包含特定的Places服務中繼資料。
