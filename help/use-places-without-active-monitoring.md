...
>[!TIP]
>
>若您的應用程式使用案例需要主動區域監視，請參閱[將Places服務搭配您自己的監視解決方案](/help/using-your-own-monitor.md)。
...
以下是使用[`BroadcastReceiver`](https://codelabs.developers.google.com/codelabs/background-location-updates-android-o/index.html?index=..%2F..index#5)的Android實作範例：
...
以下是iOS實作範例。 程式碼顯示[`CLLocationManagerDelegate`](https://developer.apple.com/documentation/corelocation/cllocationmanager?language=objc)中[`locationManager:didUpdateLocations:`](https://developer.apple.com/documentation/corelocation/cllocationmanagerdelegate/1423615-locationmanager?language=objc)方法的實作：
...
以下是iOS實作範例。 程式碼顯示[`CLLocationManagerDelegate`](https://developer.apple.com/documentation/corelocation/cllocationmanager)中[`locationManager(_:didUpdateLocations:)`](https://developer.apple.com/documentation/corelocation/cllocationmanagerdelegate/1423615-locationmanager)方法的實作：
...
請參閱[將位置內容新增至Analytics請求](use-places-with-other-solutions/places-adobe-analytics/run-reports-aa-places-data.md)以進一步瞭解此主題。

## 選用 — 當使用者處於POI中時觸發專案事件

>[!TIP]
>
>擷取Places資料的建議方式為[將Places資料附加至您的Analytics請求](#attach-places-data-to-your-analytics-requests)。
>
...
>
>在下列程式碼範例中，`NSUserDefaults` (iOS)和`SharedPreferences` (Android)是用來管理地區清單：

### Android

下列程式碼範例顯示如何處理`getNearbyPointsOfInterest` （一個`List<PlacesPOI>`）回呼中提供的結果：
...
下列程式碼範例顯示如何處理`getNearbyPointsOfInterest:limit:callback:errorCallback:` （一個`NSArray<ACPPlacesPoi *> *`）回呼中提供的結果：
...
下列程式碼範例顯示如何處理`getNearbyPoints(_ ofInterest: CLLocation, limit: UInt, callback: (([ACPPlacesPoi]?) -> Void)?, errorCallback: ((ACPPlacesRequestError) -> Void)?)` （一個`[ACPPlacesPoi]`）回呼中提供的結果：
...
此程式碼範例包含選擇性步驟[，當使用者處於POI](#trigger-entry-events-when-the-user-is-in-a-poi)時會觸發進入事件。

>[!IMPORTANT]
>
>這些片段僅&#x200B;**是**&#x200B;範例。 開發人員必須決定要如何實作功能，而且決策應考量目標作業系統建議的最佳實務。
